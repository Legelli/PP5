...
Per ottenere $\text{R}_1$ il più possibile simile a $\text{R}_2$ si è usato per la seconda un trimmer, un altro trimmer è stato usato per la resistenza $\text{R}_3$ in modo da poter controllare con precisione il valore di amplificazione del filtro.
...
\end{itemize}
Il valore di $\text{R}_3$ non è riportato tra gli altri non essendo esso un valore fisso.
...
Gli andamenti sono compatibili con quelli teorici: il modulo ha un massimo in corrispondenza di $\omega_\text{T} = \SI{111 \pm 111}{\hertz}$, valore che è stato stimato come la frequenza intermedia tra le misure corrispondenti ai due valori di $\vert \text{T}(\omega) \vert$ più elevati, compatibile con quello ricavabile da (\ref{eq:trasferimento_approx}) pari a $\omega_\text{teo} = \SI{111 \pm 111}{\hertz}$; inoltre la fase assume il valore di $-\pi / 2$ in corrispondenza di $\omega_\phi = \SI{111 \pm 111}{\hertz}$ inferito dalla retta congiungente i due punti sperimentali più vicini al valore teorico assunto della fase alla frequenza di taglio, anche questo valore risulta compatibile con quello teorico.

\subsection{Butterworth}

Come seconda misura si sceglie il valore di $\text{R}_3 = \SI{111 \pm 111}{\kilo\ohm}$, ovvero si sceglie $\text{k} = \SI{111} \approx \text{k}_\text{limite}$, cioè si realizza un filtro \emph{Butterworth}. In queste condizioni non ci si aspetta alcun picco del modulo della funzione di trasferimento. I risultati delle misure sono riportati in Tabella~\ref{tab:filtro_due}, gli andamenti del modulo e della fase della funzione di trasferimento sono riportati in Figura~\ref{fig:modulo_filtro} e in Figura~\ref{fig:fase_filtro}.
